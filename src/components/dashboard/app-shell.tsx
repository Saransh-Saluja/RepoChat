...
import { Plus, GitBranch, Compass } from "lucide-react";
    ...
    <div className="flex min-h-screen">
      <aside className="flex w-64 shrink-0 flex-col border-r border-border bg-surface">
        <Link href="/dashboard" className="flex items-center gap-2 border-b border-border px-5 py-4">
          <Compass className="size-4 text-accent" strokeWidth={2.25} />
          <span className="font-display text-[15px] font-medium tracking-tight">RepoPilot</span>
        </Link>
...
function StatusDot({ status }: { status: string }) {
  if (status === "INDEXING" || status === "PENDING") {
    return <span className="size-1.5 shrink-0 animate-pulse rounded-full bg-accent" />;
  }
  if (status === "FAILED") {
    return <span className="size-1.5 shrink-0 rounded-full bg-diff-del" />;
  }
  return <GitBranch className="size-3.5 shrink-0 text-muted/60" strokeWidth={2} />;
}
